...
const ICON = {
  ...
  exitflow: "🚪", survival: "🌱", smart: "🧠", rally: "🚀", watch: "🚨", map: "🗺️", op: "🎯", hub: "🔐",
};
const shortA = (a) => a.slice(0, 6) + "…" + a.slice(-4);
const iconOf = (id) => (id.startsWith("wallet:") ? "👤" : (ICON[id] || "▪"));
const TITLE = (id) => (id.startsWith("wallet:") ? "Wallet " + shortA(id.slice(7)) : id === "overview" ? "Overview" : id === "buyers" ? "Who's Buying" : id === "smart" ? "Smart Money" : id === "rally" ? "Who Moved the Rally" : id === "watch" ? "Insider Exit Watch" : id === "map" ? "Coordination Map" : id === "op" ? "The Operation" : id === "hub" ? "Multisig Map" : id === "about" ? "About" : (chartById(id)?.title || id));
const DEFSIZE = (id) => (id === "smart" || id === "rally" || id === "watch" || id === "map" || id === "op" || id === "hub" ? [880, 660] : id.startsWith("wallet:") ? [720, 560] : id === "overview" ? [740, 580] : id === "buyers" ? [760, 540] : id === "about" ? [480, 380] : id === "whales" || id === "clusters" ? [720, 520] : [680, 480]);

// desktop icons (curated)
const DESKTOP = ["overview", "buyers", "rally", "op", "hub", "watch", "map", "smart", "exitflow", "survival", "realized", "mvrv", "hodl", "whales", "about"];
    ...
    <>
      ...
      <div className="smwrap" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sm-scroll">
          ...
          {["overview", "buyers", "rally", "op", "hub", "watch", "map", "smart", "about"].map((id) => (
